...
@auth_bp.route('/api/auth/galleries/<int:gallery_id>/access-tokens', methods=['GET', 'POST', 'DELETE', 'OPTIONS'])
def gallery_access_tokens(gallery_id: int):
    ...
    if request.method == 'GET':
        ...
        for item in items:
            item.pop('token', None)
        return _cors_response({'success': True, 'data': {'items': items}})
    ...
    if request.method == 'POST':
        ...
        if not target_token:
            return _cors_response({'success': False, 'error': '请提供要授权的 Token'}, 400)
        expires_at = data.get('expires_at')
        success = grant_gallery_token_access(gallery_id, target_token, owner_token=token, expires_at=expires_at)
        if not success:
            return _cors_response({'success': False, 'error': '授权失败，画集不存在、无权限或 Token 无效'}, 400)
        return _cors_response({'success': True, 'message': '授权成功'})

    # DELETE
    target_token = (data.get('token') or '').strip()
    if not target_token:
        return _cors_response({'success': False, 'error': '请提供要撤销的 Token'}, 400)
    success = revoke_gallery_token_access(gallery_id, target_token, owner_token=token)
    if not success:
        return _cors_response({'success': False, 'error': '撤销失败，画集不存在、无权限或授权不存在'}, 400)
    return _cors_response({'success': True, 'message': '撤销成功'})
...
@auth_bp.route('/api/shared/galleries/<share_token>/unlock-token', methods=['POST', 'OPTIONS'])
def unlock_gallery_with_token(share_token: str):
    ...
    if gallery.get('access_mode') != 'token':
        return _cors_response({'success': False, 'error': '该画集不需要 Token 验证'}, 400)

    data = request.get_json(silent=True) or {}
    unlock_token_str = (data.get('token') or '').strip()
    if not unlock_token_str:
        return _cors_response({'success': False, 'error': '请提供 Token'}, 400)

    # 验证 Token 有效性
    verification = verify_auth_token_access(unlock_token_str)
    if not verification['valid']:
        return _cors_response({'success': False, 'error': f"Token 无效: {verification['reason']}"}, 401)

    # 检查是否为所有者或已授权
    if not (is_gallery_owner(gallery['id'], unlock_token_str) or is_token_authorized_for_gallery(gallery['id'], unlock_token_str)):
        return _cors_response({'success': False, 'error': '该 Token 未被授权访问此画集'}, 403)
...
@auth_bp.route('/api/shared/all/<share_all_token>/galleries/<int:gallery_id>', methods=['GET', 'OPTIONS'])
def shared_all_gallery_detail_api(share_all_token: str, gallery_id: int):
    ...
    if not gallery:
        return _cors_response({'success': False, 'error': '分享链接无效或画集不可见'}, 404)
...
@auth_bp.route('/api/shared/all/<share_all_token>/galleries/<int:gallery_id>/unlock', methods=['POST', 'OPTIONS'])
...
@auth_bp.route('/api/shared/all/<share_all_token>/galleries/<int:gallery_id>/unlock-token', methods=['POST', 'OPTIONS'])
...
@admin_bp.route('/api/admin/galleries/<int:gallery_id>/access-tokens', methods=['GET', 'POST', 'DELETE', 'OPTIONS'])
@login_required
def admin_gallery_access_tokens(gallery_id: int):
    ...
    if request.method == 'POST':
        ...
        if not success:
            return _cors_response({'success': False, 'error': '授权失败，画集不存在或 Token 无效'}, 400)
        return _cors_response({'success': True, 'message': '授权成功'})
    ...
    if not success:
        return _cors_response({'success': False, 'error': '撤销失败，画集不存在或授权不存在'}, 400)
    return _cors_response({'success': True, 'message': '撤销成功'})
