...
REM Windows launcher for build-multiarch.sh

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "SCRIPT_SH=%SCRIPT_DIR%\build-multiarch.sh"

echo =========================================
echo Multi-arch Docker build launcher
echo =========================================
echo.
...
if errorlevel 1 (
    echo [ERROR] docker buildx is not available. Start Docker Desktop first.
    goto :fail
)
...
if defined GIT_BASH (
    ...
    echo.
    if /I "%MULTIARCH_LAUNCHER_DRY_RUN%"=="1" (
        echo [DRY-RUN] Skipped actual execution
        goto :success
    )
    pushd "%SCRIPT_DIR%"
    call "%GIT_BASH%" -lc "./build-multiarch.sh"
...
if not errorlevel 1 (
    ...
    wsl.exe bash -lc "cd '%WSL_DIR%' && ./build-multiarch.sh"
...
:runner_fail
echo.
echo [ERROR] build-multiarch.sh failed
goto :fail
