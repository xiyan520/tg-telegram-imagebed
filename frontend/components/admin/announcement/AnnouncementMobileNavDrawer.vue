    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="text-base font-semibold text-stone-900 dark:text-white">公告设置分组</h3>
...
import type { AnnouncementDirtyMap, AnnouncementSectionItem, AnnouncementSectionKey } from '~/types/announcement'

const props = defineProps<{
  items: AnnouncementSectionItem[]
  activeKey: AnnouncementSectionKey
  dirtyMap: AnnouncementDirtyMap
}>()

const emit = defineEmits<{
  (e: 'select', key: AnnouncementSectionKey): void
}>()

const open = ref(false)
const activeItem = computed(() => props.items.find((item) => item.key === props.activeKey))

const handleSelect = (key: AnnouncementSectionKey) => {
  emit('select', key)
  open.value = false
}
</script>
